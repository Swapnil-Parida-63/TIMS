...
export const CandidateDetail = () => {
  ...
  const isFinalized    = linkedInterview?.status === 'selected';
  const isRejectedOrCancelled = ['rejected', 'cancelled'].includes(linkedInterview?.status) || candidate.status === 'rejected';
    ...
    <div className="max-w-4xl mx-auto flex flex-col gap-4 pb-12">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap justify-end">
          ...
          {candidate.status === 'applied' && canSchedule(user?.role) && (
          ...
          {!isFinalized && !isRejectedOrCancelled && ['admin', 'super_admin'].includes(user?.role) && (
            <button onClick={() => setConfirmRejectOpen(true)}
              className="px-4 py-2 text-sm font-semibold bg-white text-red-600 border border-red-200 rounded-xl hover:bg-red-50 transition">
              Reject
            </button>
          )}
      ...
      {editMode ? (
        <div className="flex flex-col gap-4">
          <div className="bg-amber-50 border border-amber-200 rounded-2xl px-4 py-3 flex items-center gap-2 text-amber-700 text-sm font-medium">
            <Pencil size={15} />
            Editing teaching profile — Basic info and family details are locked and cannot be changed.
          </div>
